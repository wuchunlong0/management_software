...
from web.models import Materialstorage
...
from web.forms.materialstorage import MaterialstorageForm
...
def materialstorage_import(request):
    """批量导入 入库"""
    down_tpl = '/web/materialstorage/tpl/'  #下载模板路径
    
    if request.method == 'GET':
        return render(request, 'web/import.html',context=locals())
    
    k = ["date", "name","receipt","product_name","number",\
         "univalence","money","remarks","operator"]  
       
    ret = post_excel_model(request, 'post_file_excel', Materialstorage, k) 
...
def materialstorage_list(request, page):
    """材料入库 列表"""
    cleanData = request.GET.dict()
    model = Materialstorage.objects
    ...
    name_list.insert(0, '')
    return render(request, 'web/materialstorage/materialstorage_list.html', context=locals())


def materialstorage_add(request):
    """添加"""
    if request.method == 'GET':
        form = MaterialstorageForm()
        return render(request, 'web/form_submit.html', context=locals())
    form = MaterialstorageForm(data=request.POST)
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Materialstorage.objects
        model_id_update(request, model, get_model_last_id(model))
        return redirect('/web/materialstorage/list/(.+)')
    return render(request, 'web/form_submit.html', context=locals())
  
  
def materialstorage_edit(request, cid):
    """编辑"""
    obj = Materialstorage.objects.get(id=cid)
    if request.method == 'GET':
        form = MaterialstorageForm(instance=obj)
        return render(request, 'web/form_submit.html', context=locals())
    form = MaterialstorageForm(data=request.POST, instance=obj)    
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Materialstorage.objects
...
def materialstorage_del(request, cid):
    """删除"""
    Materialstorage.objects.filter(id=cid).delete()
    return redirect('/web/warehousing/list/(.+)')
...
#单页保存Excel  down_file
def materialstorage_makexlsx_page(request, page):
    datas, tempFilePath, fileName = get_post_data(request, Materialstorage.objects)
