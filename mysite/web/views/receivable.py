...
from web.models import Receivable
...
from web.forms.receivable import ReceivableForm
...
def model_id_update(request, model, id):
    """" 更新一条数据库记录。
            v[6] = round(float(v[4]) * float(v[5]), 2) #金额 =  送货数量 *  单价
            if n > 0: 
                v[8] = round((v8 + v[6] - v[7]), 2) #余额 = 余额_1 + 金额 - 收款
                v[13] =  v13 +  v[6] - v[12]  #欠票 = 欠票_1 + 金额 - 金额1
                v8, v13 = v[8], v[13]    
    """    
    id = int(id)    
    ...
    if id > 1:
        ...
        balance = round(balance_1 + money - model.get(id=id).collection, 2) #余额 = 余额_1 + 金额 - 收款
...
def receivable_import(request):
    """批量导入应收账款"""
    down_tpl = '/web/receivable/tpl/'  #下载模板路径
    ...
    ret = post_excel_model(request, 'post_file_excel', Receivable, k) 
...
def receivable_list(request, page):
    """应付账款列表"""
    cleanData = request.GET.dict()
    model = Receivable.objects
    ...
    collections = round(sum(datas.filter().values_list('collection', flat=True)),2) 
    ...
    name_list = list(set(Receivable.objects.values_list('name', flat=True)))    
    return render(request, 'web/receivable/receivable_list.html', context=locals())


def receivable_add(request):
    """添加"""
    if request.method == 'GET':
        form = ReceivableForm()
        return render(request, 'web/form_submit.html', context=locals())
    form = ReceivableForm(data=request.POST)
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Receivable.objects
...
def receivable_edit(request, cid):
    """编辑"""
    obj = Receivable.objects.get(id=cid)
    if request.method == 'GET':
        form = ReceivableForm(instance=obj)
        return render(request, 'web/form_submit.html', context=locals())
    form = ReceivableForm(data=request.POST, instance=obj)    
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Receivable.objects
...
def receivable_del(request, cid):
    """删除"""
    Receivable.objects.filter(id=cid).delete()
    return redirect('/web/receivable/list/(.+)')
...
#单页保存Excel  down_file
def receivable_makexlsx_page(request, page):
    datas, tempFilePath, fileName = get_post_data(request, Receivable.objects)
    ...
    return HttpResponseRedirect(r'/web/receivable/list/%s' % (page))

#全部保存Excel
def receivable_makexlsx_all(request, page):
    datas, tempFilePath, fileName = get_post_data(request, Receivable.objects)
