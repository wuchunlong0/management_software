...
from web.models import Warehousing
...
from web.forms.warehousing import WarehousingForm
...
def warehousing_import(request):
    """批量导入 成品入库"""
    ...
    ret = post_excel_model(request, 'post_file_excel', Warehousing, k) 
...
def warehousing_list(request, page):
    """成品入库 列表"""
    cleanData = request.GET.dict()
    model = Warehousing.objects
    ...
    name_list.insert(0, '')
    return render(request, 'web/warehousing/warehousing_list.html', context=locals())


def warehousing_add(request):
    """添加 成品入库"""
    if request.method == 'GET':
        form = WarehousingForm()
        return render(request, 'web/form_submit.html', context=locals())
    form = WarehousingForm(data=request.POST)
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Warehousing.objects
...
def warehousing_edit(request, cid):
    """编辑"""
    obj = Warehousing.objects.get(id=cid)
    if request.method == 'GET':
        form = WarehousingForm(instance=obj)
        return render(request, 'web/form_submit.html', context=locals())
    form = WarehousingForm(data=request.POST, instance=obj)    
    if form.is_valid():
        form.save()
        # 更新一条数据库记录
        model = Warehousing.objects
...
def warehousing_del(request, cid):
    """删除"""
    Warehousing.objects.filter(id=cid).delete()
    return redirect('/web/warehousing/list/(.+)')
...
#单页保存Excel  down_file
def warehousing_makexlsx_page(request, page):
